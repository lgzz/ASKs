...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, cfg1, cfg2, stride=1, downsample=None):
        ...
        self.conv1 = AnyModule_d(inplanes, cfg, cfg1, stride=stride)
        self.bn1 = nn.BatchNorm2d(cfg)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = AnyModule_d(cfg, planes, cfg2)
...
class resnet_any(nn.Module):
    ...
    def __init__(self, depth, dataset='cifar10', cfg=None, anycfg='ASK_5a'):
        ...
        self.cfg = cfg
        cfg_list = eval('any_resnet%s_cfglist'%depth)[anycfg]
        self.cfg_list = []
        for i in range(len(cfg_list)):
            c = []
            for j in range(len(cfg_list[i])):
                d = []
                for k in range(len(cfg_list[i][j])):
                    d.append(pair[cfg_list[i][j][k]])
                c.append(d)
            self.cfg_list.append(c)
        self.layer = 0

        self.inplanes = 16
        self.conv1 = AnyModule_d(3, 16, self.cfg_list[self.layer])
        self.layer += 1
