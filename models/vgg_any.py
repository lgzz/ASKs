...
class vgg_any(nn.Module):
    def __init__(self, dataset='cifar10', depth=16, init_weights=True, cfg=None, anycfg='ASK_5a'):
        super(vgg_any, self).__init__()
        if cfg is None:
            cfg = defaultcfg[depth]

        self.cfg = cfg
        cfg_list = eval('any_vgg%s_cfglist'%depth)[anycfg]
        self.cfg_list = []
    ...
    def make_layers(self, cfg, batch_norm=False):
        ...
        for v in cfg:
            if v == 'M':
                ...
                conv2d = AnyModule_d(in_channels, v, self.cfg_list[layer])
                layer += 1
                if batch_norm:
